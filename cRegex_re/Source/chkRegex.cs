...
namespace cRegex {
	...
	public class tReg {
		...
		public void rComma() {
			...
			Console.Write(this.m.Count + " : ");
			for(int i = 0; i < this.m.Count; i++) Console.Write("[" + this.m[i].Value + "]");
			Console.WriteLine();
		}
		...
		public void pac2() {
			...
			for(int i = 0; i < 8; i++)
				bin[i] = (byte)(i + 0x61);
			bin[0] = 0xff; bin[1] = 7; bin[2] = 0xfe;
			this.cvtone2(bin, "second test");

			bin = new byte[8];
			for(int i = 0; i < 8; i++)
				bin[i] = (byte)(i + 0x61);
			bin[1] = 7; bin[2] = 0xff; bin[3] = 0xfe;
		...
		private void enone(string s) {
			...
			Console.Write("[" + s + "] ");
 			for(int i = 0; i < nb.Length; i++) Console.Write(" " + nb[i].ToString("x2"));
			Console.WriteLine();
		}
		public void ens() {
			this.enone("\xe5\x41");
		...
		private void brone_old(string s) {
			...
			foreach(byte b in mb) Console.Write(b.ToString("x2") + " ");
			Console.WriteLine();
